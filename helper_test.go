...
func TestClean(t *testing.T) {
	//res := codeCleanPipeline(`runtime.ReadMemStats(&m)`)
	//res := codeCleanPipeline(`for _, c := range []string{"<", ">", ")", "(", "[", "]", "|", "=", ",", ":"} {`)
	res := codeCleanPipeline(`// SPDX-License-Identifier: MIT
...
`)
	...
	for _, r := range strings.Fields(res) {
		if len(r) >= 3 {
			sum[r] = sum[r] + 1
		}
	}
